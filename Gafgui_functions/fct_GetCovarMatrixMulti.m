...
fclose(file);
[p,sycal,a1,a2,R,df] = fct_lsfmulti(DOSE,OD,s1,t0,M,type); %WB 2022: changed lsf to lsfmulti which does the sigma1 fit in it
[validity,crit] = fct_iscurvevalid(p,M,type,min(DOSE),max(DOSE));
    ...
    if Npixappl==0 %WB 2022: to be investigated why this could happen
        ...
        if single == 1
            if isempty(varargin) % absolute
                ...
                Vappl1 = diag(sycal^2+exp(a1+a2*(ntappl+t0)).^2./(Npixappl));
                % this is for same irradiation (rule 2)
                Vappl2 = sycal^2+diag(exp(a1+a2*(ntappl+t0)).^2./(Npixappl));
            elseif numel(varargin)==2 % relative
                ...
                Vappl1(end,end)  = sycal^2  + exp(a1+a2*(tnorm+t0)).^2./Npixnorm ;
                ...
                Vappl2(end,end)  = sycal^2 + exp(a1+a2*(tnorm+t0)).^2./(Npixnorm) ;
            end
            H = diag(1./(dFappl*p));
