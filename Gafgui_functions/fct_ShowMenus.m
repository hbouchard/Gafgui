...
%this confirms that we are in Gafgui...
    %Menu1: some should remain on at all times
    ...
    %Menu2: this should remain on only when there is an image
    ...
    %Menu3: this should remain on when there is no image
 ...
 %       set(handles.menus.SHOWXI, 'Enable', fct_LogicToSwitch(handles.data==5));
        set(handles.menus.SHOWDOSE, 'Enable', fct_LogicToSwitch((handles.data==3)||(handles.data==4)||(handles.data==5)));
        set(handles.menus.DEFINE_REF_DOSE, 'Enable', fct_LogicToSwitch(handles.data==4));
    %Menu5: this should remain on with image ony
    ...
    %Menu6: this should remain on at all times
    set(handles.menus.MENUCHARACT, 'Enable', 'on');
        ...
        set(handles.menus.MENUHOMOG, 'Enable', 'on');
            ...
            set(handles.menus.HOMOG_MATRXDIFF, 'Enable', 'off');%I am switching this off momentarily 17 jan 2020 
        set(handles.menus.MULTICHANNEL_CHARACTERIZE, 'Enable', fct_LogicToSwitch(fct_isthereanimage(handles)~=1));
    %Menu7: this should remain on at all times
    ...
    %Menu8: this should remain on if no image only
