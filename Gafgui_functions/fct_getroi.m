% --------------------------------------------------------------------
function [newz,center,owidth,cropz] = fct_getroi(z,delta,option,iwidth);
...
elseif  (strcmp(option,'Rectangular')||strcmp(option,'Fixed'))&&(length(iwidth)~=0)
    ...
    api.delete();
    cropz = [xindex yindex owidth(1)/delta owidth(2)/delta];
elseif  strcmp(option,'Point')&&(length(iwidth)~=0)
    [x,y] = ginput;
    if length(x)==0
       owidth = [];
       center = [];
